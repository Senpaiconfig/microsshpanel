...
clear
echo "alias proxyfixed='wget -q 'https://raw.githubusercontent.com/Senpaiconfig/microsshpanel/main/f.sh' && chmod +x f.sh && ./f.sh'" >> ~/.bashrc
...
# If the user presses Enter, exit the script
if [ -z "$input" ]; then
  echo "Exiting..."
  exit 0
fi
